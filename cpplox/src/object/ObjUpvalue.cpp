...
ObjUpvalue::ObjUpvalue(std::shared_ptr<std::unique_ptr<Value>> location)
    : location{location} {}

std::shared_ptr<std::unique_ptr<Value>> ObjUpvalue::getLocation() const {
	return location;
}

Value &ObjUpvalue::getValue() { return *location.get()->get(); }
